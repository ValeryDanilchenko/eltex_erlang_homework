...
test_take() ->
  keylist:take(?TEST_KEYLIST_NAME1, 1),
  ?assertMatch({true, {state, [{key1, "Value1", "Comment1"}], 4}},wait_result()),
  keylist:take(?TEST_KEYLIST_NAME1, 1),
  ?assertMatch({false, {state, [{key1, "Value1", "Comment1"}], 5}}, wait_result()).

test_delete() ->
  keylist:delete(?TEST_KEYLIST_NAME1, 1),
...
test_find() ->
  ...
  ?assertMatch({false, {state, [{key3, "Value3", "Commen3"}], 10}},wait_result()),
  keylist:find(?TEST_KEYLIST_NAME1, key1),
  ?assertMatch({{key3, "Value3", "Commen3"}, {state, [{key3, "Value3", "Commen3"}], 11}},wait_result()).
